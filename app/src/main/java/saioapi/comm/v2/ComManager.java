...
public class ComManager {
    ...
    public static final int XAC_DEVICE_VID = 8578;

    /** Product id of XAC Peripheral Devices. */
    public static final int XAC_DEVICE_PID = 32768;
     ...
     */
    public int getBuiltInEppDevId(){
        int epp_id;

        if (mSys.getCpuFamily() == mSys.CPU_FAMILY_QCOM)
            epp_id = DEVICE_COM1;
        else if (mSys.getCpuFamily() == mSys.CPU_FAMILY_ALLWINNER)
            epp_id = DEVICE_COM3;
        else
            epp_id = DEVICE_COM2;
     ...
     */
    public int getCardReaderDevId(){
        int epp_id;

        if (getProductName().contains("T305"))
            epp_id = DEVICE_COM2;
        else
            epp_id = ERR_OPERATION;
    ...
    public int open(int dev_id) {
        ...
        if (isUsbdevice) {
            ...
            if ((dev_id >= DEVICE_COM0 && dev_id <= DEVICE_COM4) ||(dev_id >= DEVICE_EXT_USB0 && dev_id <= DEVICE_VSP3 || dev_id == DEVICE_USB0)) {
